    ...
    >
      {items.map((item, index) => (
        ...
        >
          ...
          >
            <span
              className="m-auto w-[1em] h-[1.5em] flex items-center justify-center"
              aria-hidden="true"
            >
              {item.icon}
            </span>
          ...
          <span className="absolute top-full left-0 right-0 text-center text-white whitespace-nowrap leading-[2] text-base opacity-0 transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] translate-y-0 group-hover:opacity-100 group-hover:  [transform:translateY(20%)]">
            {item.label}
          </span>
        </button>
      ))}
